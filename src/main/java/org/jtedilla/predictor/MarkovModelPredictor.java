...
//		}
	...
	public double execute(String entityID, String record) {
		...
		if (localPredictor) {
			//local metric
            System.out.println(">>> Is recordSeq.size() [" + recordSeq.size() +"] == stateSeqWindowSize [" + stateSeqWindowSize + "]" );
			if (recordSeq.size() == stateSeqWindowSize) {
        ...
        System.out.println(" ************************   Score is:" + score + " but threashold is: " + metricThreshold);
		if (score > metricThreshold) {
			...
			stBld.append(": ");
			stBld.append(score);
            System.out.println("************ FRAUD DETECTED: " + stBld.toString() + " ************");
	...
	private double getLocalMetric(String[] stateSeq) {
        System.out.println(" >>>>>> MarkovModelPredictor:getLocalMetric");
		double metric = 0;
...
//		}
        System.out.println(">>>> MarkovModelPredictor:getLocalMetric >>>> params:" + params[0] + ":" + params[1]);
	...
	private void missProbability(String[] stateSeq, double[] params) {
		...
		if (debugOn){}
			System.out.println(" MarkovModelPredictor:missProbability >>>> params:" + params[0] + ":" + params[1]);
	}
